...
export default {
  ...
  methods: {
    async getUserDetailById() {
      const { data } = await reqGetUserDetailById(this.userId) // 获取是，要赋值头像
      this.$refs.staffPhoto.fileList = [{ url: data.staffPhoto }] // 将员工头像显示在上传组件中s
      this.userInfo = data
    },
    // 上面的信息保存
    async saveUser() {
    ...
    async getPersonalDetail() {
      ...
      this.$refs.picPhoto.fileList = [{ url: data.staffPhoto }] // 将员工头像显示在上传组件中
      this.formData = data
    },
    // 下面的信息保存
    async savePersonal() {
